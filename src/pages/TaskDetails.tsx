             ...
             <div className="p-8 space-y-4">
                ...
                <div className="space-y-3">
                   {task.subtasks?.map(st => (
                      ...
                      >
                         <div className="flex items-center gap-4">
                            <div className={`p-1 rounded-full transition ${st.is_completed ? 'text-green-500' : 'text-gray-200 group-hover:text-gray-300'}`}>
                               {st.is_completed ? <CheckCircle2 className="w-8 h-8" /> : <Circle className="w-8 h-8" />}
                            </div>
                            <span className={`text-lg font-bold transition ${st.is_completed ? 'text-gray-300 line-through' : 'text-gray-700'}`}>
                               {st.description}
                            </span>
                         </div>
                      </div>
                   ...
                   {(!task.subtasks || task.subtasks.length === 0) && (
                      <div className="py-12 text-center text-gray-300 italic font-bold">لم يتم إضافة خطوات بعد.</div>
                   )}
                </div>
             </div>
          </div>
            ...
            <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 p-8 space-y-6">
               ...
               <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                  {payments && payments.length > 0 ? (
                    ...
                    <div className="py-8 text-center text-gray-300 italic font-bold text-sm">لا يوجد سجل مدفوعات بعد.</div>
                  )}
               </div>
            </div>
         </div>
