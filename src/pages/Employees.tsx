  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-in fade-in duration-700 pb-20">
      ...
      <div className="bg-bg-surface rounded-[2.5rem] shadow-sm border border-border-theme overflow-hidden">
        <div className="p-8 border-b border-border-theme flex flex-col md:flex-row justify-between gap-6 items-center bg-bg-primary/20">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 p-8">
          {filteredEmployees.map((emp) => (
            <div key={emp.id} className="bg-bg-primary/30 border border-border-theme/50 rounded-[2rem] p-6 hover:shadow-xl transition duration-500 group relative">
              ...
              <h3 className="text-xl font-bold text-text-primary mb-1">{emp.name}</h3>
              <p className="text-brand-main dark:text-brand-secondary font-bold text-sm mb-6 flex items-center gap-2">
                <Briefcase className="w-4 h-4" />
                {emp.role}
              </p>

              <div className="grid grid-cols-2 gap-4 pt-6 border-t border-border-theme/50">
                <div>
                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">أجر الساعة</p>
                  <p className="font-bold text-text-primary">{emp.hourly_rate} جنية</p>
                </div>
                <div>
                  <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest mb-1">الحالة</p>
                  <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold uppercase ${
                    emp.status === 'Active' ? 'bg-green-100/10 text-green-500' : 'bg-bg-primary text-text-muted'
                  }`}>
              ...
              <Link 
                to={`/employees/${emp.id}`}
                className="mt-8 w-full py-4 bg-bg-surface border border-border-theme rounded-2xl font-bold text-sm text-text-secondary hover:bg-brand-main hover:text-brand-third dark:hover:bg-brand-secondary dark:hover:text-brand-main hover:border-transparent transition-all flex items-center justify-center gap-2 shadow-sm"
              >
